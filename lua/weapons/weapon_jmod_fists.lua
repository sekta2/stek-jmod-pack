...
if CLIENT then
    local HandTex, ClosedTex = surface.GetTextureID("vgui/hud/gmod_hand"),
        surface.GetTextureID("vgui/hud/gmod_closedhand")
    ...
    function SWEP:DrawHUD()
        ...
        if not self:GetFists() then
            local Tr = util.QuickTrace(ply:GetShootPos(), ply:GetAimVector() * self
                .ReachDistance,
                { ply })
            ...
            if Tr.Hit then
                if self:CanPickup(Tr.Entity) then
                    local Size = math.Clamp(
                        1 - ((Tr.HitPos - ply:GetShootPos()):Length() / self.ReachDistance) ^ 2, .2, 1)
...
function SWEP:ApplyForce()
    ...
    if IsValid(phys) then
        ...
        local StandingOn = IsValid(StandingEnt) and
            ((StandingEnt == self.CarryEnt) or (StandingEnt:IsConstrained() and table.HasValue(constraint.GetAllConstrainedEntities(StandingEnt), self.CarryEnt)))
